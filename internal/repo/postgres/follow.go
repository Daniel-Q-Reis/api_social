...
func (r *FollowRepo) Exists(ctx context.Context, userID, followerID uuid.UUID) (bool, error) {
	var (
		exists bool
		query  = `SELECT EXISTS(SELECT 1 FROM followers WHERE user_id = $1 AND followerID = $2)`
	)
...
func (r *FollowRepo) GetFollowers(ctx context.Context, userID uuid.UUID, limit, offset int) ([]entity.User, error) {
	return r.getUsersByRelation(ctx, userID, limit, offset, true)
}

func (r *FollowRepo) GetFollowing(ctx context.Context, followerID uuid.UUID, limit, offset int) ([]entity.User, error) {
	return r.getUsersByRelation(ctx, followerID, limit, offset, false)
}

func (r *FollowRepo) getUsersByRelation(ctx context.Context, userID uuid.UUID, limit, offset int, isFollowers bool) ([]entity.User, error) {
	var query string
	if isFollowers {
		query = `
